...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
				...
				<fo:static-content flow-name="xsl-region-before">
					<xsl:call-template name="pdf-entete">
						<xsl:with-param name="titre">
							Feuille de match des phases éliminatoires
						</xsl:with-param>
					</xsl:call-template>
				</fo:static-content>
					...
					<fo:block font-size="10pt">
						...
						<xsl:choose>
							...
							<xsl:when test="$idCategorie = ''">
								<xsl:apply-templates select="/concours/listeCategories/categorie/listePhasesEliminatoires/phaseEliminatoire/matchPhaseEliminatoire[count(./participation[@resultat = 'attente']) != 0]">
								</xsl:apply-templates>
							</xsl:when>
							<!-- Catégorie existante -->
							<xsl:when test="count(./concours/listeCategories/categorie[@id=$idCategorie]) = 1">
								<xsl:apply-templates select="/concours/listeCategories/categorie[@id=$idCategorie]/listePhasesEliminatoires/phaseEliminatoire/matchPhaseEliminatoire[count(./participation[@resultat = 'attente']) != 0]">
								</xsl:apply-templates>
							</xsl:when>
	...
	<!-- Template d'un match -->
	<xsl:template match="matchPhaseEliminatoire">
