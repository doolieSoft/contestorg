...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<html>
			<!-- Head -->
			<head>
				<title>Résultats de la phase qualificative <xsl:value-of select="//phaseQualificative[@id=$idPhase]/@numero+1" /></title>
			...
			<body onload="setTimeout('scrollit()', 3000);">
				<xsl:choose>
					...
					<xsl:when test="count(//phaseQualificative[@id=$idPhase]) = 1">
						...
						<h3>Résultats de la phase qualificative <xsl:value-of select="//phaseQualificative[@id=$idPhase]/@numero+1" /></h3>
