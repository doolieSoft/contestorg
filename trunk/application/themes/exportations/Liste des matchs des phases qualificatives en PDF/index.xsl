...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:param name="uniquementMatchsNonJoues" />
				...
				<fo:static-content flow-name="xsl-region-before">
					<xsl:call-template name="pdf-entete">
						<xsl:with-param name="titre">
							<!-- Titre -->
							Liste des matchs des phases qualificatives
						</xsl:with-param>
					</xsl:call-template>
				</fo:static-content>
	...
	<xsl:template match="categorie">
		<xsl:if test="count(./listePoules/poule/listePhasesQualificatives/phaseQualificative/matchPhaseQualificative[$uniquementMatchsNonJoues = 0 or count(participation[@resultat = 'attente']) != 0]) != 0">
			<!-- Titre -->
	...
	<xsl:template match="poule">
		<xsl:if test="count(./listePhasesQualificatives/phaseQualificative/matchPhaseQualificative[$uniquementMatchsNonJoues = 0 or count(participation[@resultat = 'attente']) != 0]) != 0">
			<!-- Titre -->
	...
	<xsl:template match="phaseQualificative">				
		<xsl:call-template name="pdf-titre-h3">
		  <xsl:with-param name="titre">Phase qualificative <xsl:value-of select="position()" /></xsl:with-param>
		</xsl:call-template>
		...
		<fo:block>
			<fo:table width="100%" border="0.5pt solid black">
				...
				<fo:table-body>
					<xsl:apply-templates select="./matchPhaseQualificative[$uniquementMatchsNonJoues = 0 or count(participation[@resultat = 'attente']) != 0]">
					</xsl:apply-templates>
				</fo:table-body>
			</fo:table>
		</fo:block>
