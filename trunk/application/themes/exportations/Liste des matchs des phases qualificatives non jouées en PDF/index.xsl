...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
				...
				<fo:static-content flow-name="xsl-region-before">
					<xsl:call-template name="pdf-entete">
						<xsl:with-param name="titre">
							<!-- Titre -->
							Liste des matchs des phases qualificatives non jouées
						</xsl:with-param>
					</xsl:call-template>
				</fo:static-content>
	...
	<xsl:template match="categorie">
		<xsl:if test="count(listePoules/poule/listePhasesQualificatives/phaseQualificative[count(matchPhaseQualificative/participation[@resultat != 'attente']) = 0]) != 0">
			<!-- Titre -->
	...
	<xsl:template match="poule">
		<xsl:if test="count(./listePhasesQualificatives/phaseQualificative[count(matchPhaseQualificative/participation[@resultat != 'attente']) = 0]) != 0">
			<!-- Titre -->
			...
			<!-- Liste des phases qualificatives non jouées -->
	...
	<xsl:template match="phaseQualificative">
		<xsl:if test="count(matchPhaseQualificative/participation[@resultat != 'attente']) = 0">
